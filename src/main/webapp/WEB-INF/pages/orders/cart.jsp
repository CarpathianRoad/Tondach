...
<t:orderspage>
    <div class="page-header main-page-title">
      <div class="container">
        <h3>Кошик замовлень<c:if test="${sessionScope.carts == null}">  <a type="button" style="margin-left: 50px;" class="btn btn-primary show-items" href="<c:url value="/orders/goods" />">Створити нове замовлення</a></c:if></h3>
      </div>
    </div>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <table class="table table-hover table-bordered">
                            ...
                            <tbody>
                                <c:forEach var="item" items="${sessionScope.carts}">
                                    <tr>
                                      ...
                                      <td class="item-remove text-center"><a href="<c:url value="/orders/system/remove/${item.item_code}" />" title="Видалити позицію"><span class="fa fa-times"></span></a></td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                            ...
                            <form name="login-form" action="<c:url value="/orders/complete" />" method="POST">
                                <label for="comment">Примітка:</label>
                                <textarea class="form-control" rows="5" id="comment" name="descr"></textarea>
                            <a type="button" style="float: left;margin-top: 11px;" class="btn btn-primary show-items" href="<c:url value="/orders/goods" />">Повернутись до вибору товарів</a>
                            <div class="order-control" style="float: right;"> 
                                <button href="<c:url value="/orders/complete" />" style="    margin-top: 10px;" class="btn btn-success btn-mini sudmitData" type="submit" data-dismiss="modal">Відправити замовлення</button>
                            </div>
                            </form>
                         </c:if>
                    </div>
